...
void SoundAnnounce(int sound)
{
		if (!announcer->value)
			return;

		if (lastannounce >= level.time)
			return;
		else
			lastannounce = level.time + 5;

		if (sound==1)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/team1w.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==2)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/team2w.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==3)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/draw.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==4)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/headshot.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==5)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/stomach.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==6)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/prof.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==7)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/1min.wav"), 1.0, ATTN_NONE, 0.0);
		else if (sound==8)
	        gi.sound(&g_edicts[0], CHAN_VOICE | CHAN_NO_PHS_ADD, gi.soundindex("ltk/5min.wav"), 1.0, ATTN_NONE, 0.0);
}
