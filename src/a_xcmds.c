...
void Cmd_Voice_f(edict_t *self)
{
  ...
  if (self->deadflag == DEAD_DEAD || self->solid == SOLID_NOT) return;
...
void Cmd_BeginCube_f(edict_t* self)
{
	if(setcube)
	{
...
void Cmd_SetCubeLL_f(edict_t* self)
{
	if(!setcube)
	{
...
void Cmd_SetCubeUR_f(edict_t* self)
{
	if(!setcube)
	{
		gi.cprintf(self, PRINT_MEDIUM, "Please allocate a cube first by executing BeginCube.\n");
...
void Cmd_AbortCube_f(edict_t* self)
{
	if(!setcube)
	{
...
void Cmd_AddCube_f(edict_t *self)
{
  FILE *pntlist;
  char *s, buf[1024];
...
void Cmd_PrintCubeState_f(edict_t* self)
{
  if(!setcube)
  {
	gi.cprintf(self, PRINT_MEDIUM, "\nPlease allocate a cube first by executing BeginCube.\n");
