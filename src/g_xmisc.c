...
void punch_attack (edict_t * ent )
{
      ...
      if (tr.ent->takedamage)            
      {
        if (teamplay->value)
        {
          if (lights_camera_action)
            return;                                      
          if (tr.ent != ent && tr.ent->client && ent->client && !ent->is_bot &&
        ...
        T_Damage (tr.ent, ent, ent, forward, tr.endpos, tr.plane.normal, damage, kick, 0, MOD_PUNCH );
        ...
        {                                        
          ...
          if (IsFemale(tr.ent))
          {
            gi.cprintf(ent, PRINT_HIGH, "You hit %s's %s from her hands!\n", tr.ent->client->pers.netname, (tr.ent->client->pers.weapon)->pickup_name );
          }
          else
          {
            gi.cprintf(ent, PRINT_HIGH, "You hit %s's %s from his hands!\n", tr.ent->client->pers.netname, (tr.ent->client->pers.weapon)->pickup_name );
          }
  ...
  //TempFile - BEGIN
  //punch animation moved to here from Cmd_Punch_f()
...
char *strtostr2(char *s)
{
  ...
  while (*p) 
  {
    if ( (*p >= 0x1b && *p <= 0x7f) || ( (*p > 0x0a && *p <= 0x11) && (*p != 0x0d) ) )
    *p += 0x80;
...
void PrecacheUserSounds(void)
{
  ...
  else
  {
    ...
    for (i = 0; (fgets(buf, MAX_STR_LEN - 10, soundlist) != NULL) && (i < 100);)
    {
      ...
      for (bs = strlen(buf); bs > 0 && (buf[bs-1] == '\r' || buf[bs-1] == '\n' || buf[bs-1] == ' '); bs--)
        buf[bs-1] = '\0';
...
void KickClient(edict_t *target, char *reason)
{
  ...
  if (target && target->client && target->inuse)
  {
    ...
    else gi.dprintf("Error: %s has to be kicked, cannot determine userid.",target->client->pers.netname);
  }
}
...
{  
  if (parse->inparsing == true) return false;
  ...
  if (parse->pfile)
  {
   ...
   else gi.dprintf("Error opening file %s\n",filename);
  return false;
}
