...
/*                else
                {
					//Werewolf: Adde this whole timedisplay stuff here
//						if (!teamplay->value)
				        if (!team_round_going)
				        {
			                if (timelimit->value)
							{
								...
								ent->client->ps.stats[STAT_TIMER] = (timelimit->value*60)-level.time;
							}
			                if (team_round_countdown)
							{
								ent->client->ps.stats[STAT_TIMER_ICON] = 0;
								ent->client->ps.stats[STAT_TIMER] = team_round_countdown / 10;
							}
						}
		                else
				        {
							...
							{
								ent->client->ps.stats[STAT_TIMER_ICON] = 0;
								ent->client->ps.stats[STAT_TIMER] = (roundtimelimit->value * 600) - current_round_length;
							}					
