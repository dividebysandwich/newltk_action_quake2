...
int _istourneysection(char *atoken)
{
...
void _tourneysetsection(tourneyindex_t *ti, int clevel)
{
  switch (clevel)
  {
    case 1:
  ...
  while (toknr < 3 && (mytok = ParseNextToken(&parse, STDSEPERATOR)))
  {
    switch (toknr)
    {
      case 0: //"starttime" or "roundstart" expected
        ...
        if (Q_stricmp(mytok, "roundstart") == 0)
        {
          if (clevel != 1)
          {
...
void TourneyTimeEvent(TOURNEYTIME ttime, int attime)
{
  ...
  for (i = 0; i < t_eventcount; i++)
  {
    if (t_eventlist[i].ttime == ttime && t_eventlist[i].attime == attime)
    {
      ...
      if (t_eventlist[i].taction == A_PRINT)
      {
...
void TourneyReadIni(void)
{
  ...
  t_eventcount = 0;  clevel = 0; inevent = false;
...
void TourneyWinner(edict_t *player)
{
  ...
  if (dummy) dummy->client->resp.team = NOTEAM;
