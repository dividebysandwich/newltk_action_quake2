...
void FixCubeData(loccube_t* cube)
{
	float tmp;
	int i;
...
void DescListInit(char *mapname)
{
  ...
  while(fgets(buf, 1000, pntlist) != NULL)
  {
    ...
    if (bs > 0 && strncmp(buf, "#", 1) != 0 && strncmp(buf, "//", 2) != 0) 
    {
	  ...
	  j = 0;
	  for (bs = 0; bs <= strlen(buf) && j < 6; bs++) if (buf[bs] == ' ') j++;
	  j = 0;
	  while ((bs < strlen(buf)) && (j < LOC_STR_LEN-1)) mapdescex[i].desc[j++] = buf[bs++];
...
int DescListCube(vec3_t origin)
{
	int i;
	loccube_t* cube;	
...
qboolean GetPositionText(vec3_t origin, char *buf)
{
	int i;
	char firstword[64];
...
void GetViewedPosition(edict_t *ent, char *buf)
{
  ...
  if (GetPositionText(tr.endpos, buf) == false)
  {
    ...
    //y width (NORTH, SOUTH) is twice greater than x width (EAST, WEST)
	  ...
	  if (rel_pos[1] > 0.0) rel_xy_pos |= RP_NORTH;
	  else if (rel_pos[1] < 0.0) rel_xy_pos |= RP_SOUTH;
	  
	  if (rel_pos[0] > 0.0) rel_xy_pos |= RP_EAST;
    else if (rel_pos[0] < 0.0) rel_xy_pos |= RP_WEST;
                     
    //creating the text message, regarding to rel_xy_pos
    strcpy(buf, "in the ");
    if (rel_xy_pos & RP_NORTH)  strcat(buf, "north");
	  if (rel_xy_pos & RP_SOUTH)  strcat(buf, "south");
	  if (rel_xy_pos & RP_EAST)  strcat(buf, "east");
	  if (rel_xy_pos & RP_WEST)  strcat(buf, "west");
    //gi.dprintf ("rel_xy_pos: %i\n", rel_xy_pos);
    //last but not least, the height, limit for up/down: 64
    if (fabs(rel_pos[2]) > 64.0)
    {
      if (rel_pos[2] < 0.0) strcat(buf, ", up");
      else strcat(buf, ", down");
	  } else strcat(buf, ", same height");    
  }
}


void GetOwnPosition(edict_t *self, char *buf)
{
  ...
  if (the_enemy && the_enemy->client)
  {
    if (GetPositionText(the_enemy->s.origin, buf) == false)
    {
	  ...
	  if (rel_xy_pos & RP_WEST)  strcat(buf, "west");
          ...
          if (fabs(rel_pos[2]) > 64.0)
          {
            if (rel_pos[2] < 0.0) strcat(buf, ", above me");
            else strcat(buf, ", under me");
	  } else strcat(buf, ", on the same level");
